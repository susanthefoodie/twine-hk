      ...
      {[1, 2, 3, 4, 5].map((n) => (
        <span
          ...
          style={{
            ...
            color: value >= n ? '#ffa500' : 'rgba(0,0,0,0.12)',
  ...
  return (
    <>
      ...
      >
        ...
        <h3 style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '18px', letterSpacing: '-0.01em', color: '#0f0f0f', margin: '0 0 4px' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(15,15,15,0.35)', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 10px' }}>
            Your rating
          </p>
        ...
        <div style={{ marginBottom: '20px' }}>
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(15,15,15,0.35)', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 8px' }}>
            Leave a note
          </p>
          <textarea
            value={note}
        ...
        <button
          ...
          style={{
            ...
            background: saving ? '#f8f8f8' : 'linear-gradient(135deg, #ff6b35, #ffa500)',
            border: 'none',
            borderRadius: '9999px',
            fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: '15px',
            letterSpacing: '0.02em',
            color: saving ? 'rgba(15,15,15,0.35)' : '#fff',
...
function SavedCard({
  ...
  return (
    ...
    >
      {/* Photo */}
      <div style={{ position: 'relative', height: '160px', background: '#ffffff' }}>
        {src ? (
        ...
        {/* Gradient */}
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, transparent 60%, rgba(250,249,247,0.85) 100%)', pointerEvents: 'none' }} />
        {/* Visited pill */}
        {saved.is_visited && (
          <div style={{ position: 'absolute', top: '8px', left: '8px', background: 'rgba(250,249,247,0.9)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '9999px', padding: '3px 9px', backdropFilter: 'blur(4px)' }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: '#10b981', letterSpacing: '0.06em', fontWeight: 600 }}>Visited ✓</span>
          </div>
        )}
        {/* Gem badge */}
        {place.hidden_gem && (
          <div style={{ position: 'absolute', top: '8px', right: '8px', background: 'rgba(250,249,247,0.9)', border: '1px solid rgba(255,107,53,0.4)', borderRadius: '9999px', padding: '3px 9px', backdropFilter: 'blur(4px)' }}>
      ...
      <div style={{ padding: '12px 14px' }}>
        <p style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '14px', letterSpacing: '-0.01em', color: '#0f0f0f', margin: '0 0 4px', lineHeight: 1.3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {place.name ?? 'Unknown Restaurant'}
        </p>
        <p style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'rgba(15,15,15,0.35)', margin: '0 0 10px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }} onClick={(e) => e.stopPropagation()}>
          <a
            ...
            title="Directions"
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '28px', background: '#f8f8f8', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '8px', textDecoration: 'none', fontSize: '14px' }}
          >
            🗺
          </a>
          <button
            ...
            title="Rate visit"
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '28px', background: '#f8f8f8', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}
          >
            ★
          </button>
          <button
...
function ListSection({
  ...
  return (
    <div style={{ marginBottom: '28px' }}>
      ...
      >
        <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '16px', letterSpacing: '-0.01em', color: '#0f0f0f' }}>
          {name} <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'rgba(15,15,15,0.35)', fontWeight: 400 }}>({items.length})</span>
        </span>
        <motion.span animate={{ rotate: open ? 180 : 0 }} style={{ color: 'rgba(15,15,15,0.35)', fontSize: '14px' }}>▾</motion.span>
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
...
function EmptyState({ onCta, isFiltered }: { onCta: () => void; isFiltered: boolean }) {
  return (
    <div style={{ textAlign: 'center', padding: '64px 0' }}>
      ...
      <h2 style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '20px', letterSpacing: '-0.02em', color: 'rgba(15,15,15,0.55)', margin: '0 0 8px' }}>
        {isFiltered ? 'No places match this filter.' : 'Your gem collection is empty.'}
      </h2>
      {!isFiltered && (
        <p style={{ fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: '14px', color: 'rgba(15,15,15,0.35)', margin: '0 0 28px' }}>
...
export default function SavedPage() {
  ...
  return (
    <div style={{ background: '#faf9f7', minHeight: '100vh', color: '#0f0f0f', fontFamily: 'var(--font-sans)', paddingBottom: '56px' }}>

      {/* ── Header ── */}
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '32px clamp(16px, 4vw, 40px) 0' }}>
        <button
          onClick={() => router.push('/home')}
          style={{ background: 'none', border: 'none', color: 'rgba(15,15,15,0.35)', cursor: 'pointer', fontSize: '18px', padding: '0 0 16px', display: 'block', lineHeight: 1 }}
        >
          ←
        </button>
        <h1 style={{ fontFamily: 'var(--font-sans)', fontWeight: 800, fontSize: '28px', letterSpacing: '-0.03em', color: '#0f0f0f', margin: '0 0 6px' }}>
          Saved Gems 💎
        </h1>
        <p style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'rgba(15,15,15,0.35)', margin: '0 0 28px', letterSpacing: '0.06em' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', gap: '8px' }}>
          <div style={{ display: 'flex', gap: '6px', background: 'rgba(0,0,0,0.04)', borderRadius: '9999px', padding: '4px', border: '1px solid rgba(0,0,0,0.08)' }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: active ? '#fff' : 'rgba(15,15,15,0.5)',
                    cursor: 'pointer',
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: active ? 'rgba(255,107,53,0.15)' : 'rgba(0,0,0,0.04)',
                    border: `1px solid ${active ? 'rgba(255,107,53,0.35)' : 'rgba(0,0,0,0.08)'}`,
                    borderRadius: '9999px',
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <input
                    ...
                    style={{
                      ...
                      borderRadius: '9999px',
                      color: '#0f0f0f', padding: '0 14px',
                      fontFamily: 'var(--font-sans)', fontSize: '13px', outline: 'none', width: '160px',
                    }}
                  />
                  <button onClick={() => setListInput(false)} style={{ background: 'none', border: 'none', color: 'rgba(15,15,15,0.35)', cursor: 'pointer', fontSize: '20px', lineHeight: 1 }}>×</button>
                </div>
              ) : (
                <button
      ...
      <AnimatePresence>
        {toast && (
          ...
          >
            <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: '13px', color: 'rgba(15,15,15,0.55)' }}>
              Removed <strong style={{ color: '#0f0f0f', fontWeight: 600 }}>{toast.name}</strong>
            </span>
            <button
              onClick={handleUndo}
